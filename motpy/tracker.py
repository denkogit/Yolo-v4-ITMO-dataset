    ...
    def active_tracks(self,
        ...
        for tracker in self.trackers:
            ...
            if cond1 and cond2 and cond3:
                ...
                #print("Box from tracker", tracker.box)
                #botleneck
                xmin, ymin, xmax, ymax = tracker.box
                xcentre = int((xmin + xmax) / 2)
                ycentre = int((ymin + ymax) / 2)

                tracks.append(Track(id=tracker.id, box=tracker.box, score=tracker.score, centroid=[xcentre, ycentre]))
                #tracks.append(Point(x=1, y=2))
